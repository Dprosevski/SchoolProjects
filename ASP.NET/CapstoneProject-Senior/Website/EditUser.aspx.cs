...
namespace Capstone2nd
{
    public partial class EditUser : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM cap.Person WHERE email = @userToEdit;", con);
            cmd.Parameters.Add(new SqlParameter("@userToEdit", Session["userToEdit"]));
        ...
        {
            string emailCurr = Session["userToEdit"].ToString();
            string confirmEmailCurr = Session["userToEdit"].ToString();
            Session["userToEditPass"] = password.Text;
            Session["userToEditAltPass"] = altPass.Text;
            Session["userToEditParentPass"] = parentPass.Text;
        ...
        protected void Confirm_Click(object sender, EventArgs e)
        {
            ...
            string passwordForm = Session["userToEditPass"].ToString();
            string altEmailForm = altEmail.Text;
            string altPassForm = Session["userToEditAltPass"].ToString();
            string phoneForm = phone.Text;
            string addressForm = address.Text;
            string genderForm = radioGender.Text;
            string dobForm = dob.Text;
            string parentPassForm = Session["userToEditParentPass"].ToString();
            ...
            try
            {
                ...
                sql = "UPDATE cap.Person SET prefix = @prefix, firstName = @firstName, middleName = @middleName, lastName = @lastName, " +
                    "sufix = @sufix, password = @password, altEmail = @altEmail, altPassword = @altPassword, phone = @phone," +
